variable "iso_checksum" {
  type    = string
  default = "sha256:101bc813d2af9ccf534d112cbe8670e6d900425b297d1a4d2529c5ad5f226372"
}
...
variable "iso_url" {
  type    = string
  default = "https://vault.centos.org/7.8.2003/isos/x86_64/CentOS-7-x86_64-NetInstall-2003.iso"
}
...
variable "vm_name" {
  type    = string
  default = "centos-7.8.2003-x64-us-server-template"
}
...
locals {
  template_description = "CentOS 7.8.2003 template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => localuser:password"
}

source "proxmox-iso" "centos7" {
  boot_command = [
     "<tab> text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos-7-preseed.cfg<enter><wait>"
  ]
