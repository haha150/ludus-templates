variable "iso_checksum" {
  type    = string
  default = "sha256:c67876a5602faa17f68b40ccf2628799b87454aa67700f0f57eec15c6ccdd98c"
}
...
variable "iso_url" {
  type    = string
  default = "http://mirror.nsc.liu.se/centos-store/centos/8.2.2004/isos/x86_64/CentOS-8.2.2004-x86_64-boot.iso"
}
...
variable "vm_name" {
  type    = string
  default = "centos-8.2.2004-x64-us-server-template"
}
...
locals {
  template_description = "CentOS 8.2.2004 template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => localuser:password"
}

source "proxmox-iso" "centos8" {
  boot_command = [
     "<tab> text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos-8-preseed.cfg<enter><wait>"
  ]
...
build {
  sources = ["source.proxmox-iso.centos8"]
